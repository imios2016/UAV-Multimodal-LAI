# -*- coding: utf-8 -*-
"""
TabPFN-DML Causal Inference Analysis for Maize LAI (GBK Effects)
Local offline version with cross-fitting stability analysis.
"""
...
def save_figure_ijors(fig, filename_base, output_dir=FIGURES_DIR):
    try:
        ...
        log(f"  Figure saved (IJoRS): {filename_base}.png/.eps/.pdf")
...
def create_tabpfn_instance(random_state=42):
    try:
        ...
        log(f"  Legacy TabPFN ({TABPFN_VERSION}), using seed", level='warning')
        return TabPFNRegressor.create_default_for_version(
            ModelVersion.V2,
            seed=random_state
        )
...
")
            if sig == 'ns':
                conclusion = f"Compared to {reference_group.replace('GBK_', '')}, {trt} shows no significant causal effect on maize LAI (p={p})."
            elif ate > 0:
                conclusion = f"Compared to {reference_group.replace('GBK_', '')}, {trt} significantly increases maize LAI by {ate:.4f} units (95% CI: [{ci_lower:.4f}, {ci_upper:.4f}], p={p})."
            else:
                conclusion = f"Compared to {reference_group.replace('GBK_', '')}, {trt} significantly decreases maize LAI by {abs(ate):.4f} units (95% CI: [{ci_lower:.4f}, {ci_upper:.4f}], p={p})."
...
")
    ...
    log(f"  DML results TXT saved: {DML_RESULTS_TXT}")
...
{'='*70}")
    ...
    n = len(residuals)
    if n < 10:
        log(f"  Warning: Insufficient samples ({n}), skipping", level='warning')
        return None, None
    ...
    text_result = f"""
To evaluate prediction error characteristics of {model_name}, residuals were analyzed.
Shapiro-Wilk test (W = {shapiro_w:.3f}, p = {shapiro_p:.3f} {'>' if shapiro_p>0.05 else '<'} 0.05)
{'supports' if shapiro_p>0.05 else 'does not support'} normal distribution assumption (mu={res_mean:.3f}, sigma={res_std:.3f}).
Empirical distribution fits theoretical normal curve well (R2={r2_fit:.3f}), with low fitting error (RMSE={rmse_fit:.3f}).
"""
    with open(RESIDUAL_STATS_FILE, 'w', encoding='utf-8') as f:
        f.write("="*80 + "
")
        f.write(f"Residual Analysis - {model_name}
")
        f.write("="*80 + "

")
        f.write(f"Time: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}
")
        f.write(f"Samples: {n}
...
")
        f.write(f"   Mean: {res_mean:.6f}
")
        f.write(f"   Std: {res_std:.6f}
")
...
")
        f.write("2. Shapiro-Wilk Test
")
        f.write(f"   W: {shapiro_w:.6f}
")
        f.write(f"   p: {shapiro_p:.6f}
")
        f.write(f"   Conclusion: {'Normal' if shapiro_p>0.05 else 'Non-normal'}

")
        f.write("3. Empirical vs Normal Fit
")
        f.write(f"   R2: {r2_fit:.6f}
")
        f.write(f"   RMSE: {rmse_fit:.6f}
...
def recode_gbk_reference(X, feature_names, new_ref='GBK_TEM'):
    log(f"
{'='*70}")
    log(f"Switching GBK reference to: {new_ref}")
    log(f"{'='*70}")
    ...
    if new_ref not in present_gbk:
        raise ValueError(f"Reference {new_ref} not found, available: {present_gbk}")
...
{'='*70}")
    ...
    log(f"  Removed: {removed_features}")
    log(f"  Retained: {len(kept_features)}")
    return new_X, kept_features

def load_full_data(model_path, data_path, test_size=TEST_SIZE, random_state=42):
    log(f"
{'='*70}")
    log("Step 1: Loading data and pretrained model")
    log(f"{'='*70}")
    ...
    if missing_keys:
        raise ValueError(f"PKL missing keys: {missing_keys}")
    ...
    log(f"  Model: {type(trained_model).__name__}")
    log(f"  random_state: {trained_model.random_state}")
    log(f"  Expected features: {n_features}")
    log(f"  Algorithm: {algorithm}")
    log(f"  Training time: {training_time}")
    log(f"  Performance: R2={performance.get('R2_mean', 'N/A'):.3f}, RMSE={performance.get('RMSE_mean', 'N/A'):.3f}")
    log(f"  Features: {model_feature_names}")
    log(f"
  Loading CSV: {data_path}")
    df = pd.read_csv(data_path)
    log(f"  CSV shape: {df.shape}")
    missing_features = [f for f in model_feature_names if f not in df.columns]
    if missing_features:
        raise ValueError(f"CSV missing features: {missing_features}")
    X = df[model_feature_names].values
    y = df['LAI'].values
    log(f"  Extracted features: {X.shape[1]}")
    log(f"  Target: LAI, samples: {len(y)}")
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=test_size, random_state=random_state
    )
    log(f"  Train: {X_train.shape[0]}, Test: {X_test.shape[0]}")
    log(f"  Using pretrained StandardScaler")
...
def evaluate_first_stage_tabpfn(X_train, y_train, X_test, y_test, feature_names, treatment_names,
    ...
    log(f"
{'='*70}")
    log("Step 2: First-stage prediction model")
    log(f"{'='*70}")
    model_name = "TabPFN"
    if tabpfn_model is not None:
        model = tabpfn_model
        log(f"  Using pretrained TabPFN")
    else:
        model = create_tabpfn_instance(42)
        log(f"  Creating new TabPFN")
    cv = KFold(n_splits=CV_FOLDS, shuffle=True, random_state=42)
    log("  Evaluating LAI prediction...")
    ...
    if not SKIP_GBK_EVAL:
        for t_name in treatment_names:
            if t_name not in feature_names:
                log(f"  Warning: {t_name} not in features, skipping", level='warning')
                continue
            log(f"  Evaluating {t_name} prediction...")
...
def plot_first_stage_performance(perf_df):
    ...
    ax.set_xlabel('First-Stage Model', fontsize=IJoRS_CONFIG['font_size_label']+1)
    ax.set_xticks(x)
    ax.set_xticklabels(models, fontsize=IJoRS_CONFIG['font_size_tick'])
    ax.set_title('First-Stage Performance', fontsize=IJoRS_CONFIG['font_size_title']+1, pad=10)
...
{'='*70}")
    ...
    for t_name in gbk_treatments:
        ...
        scenarios.append({
            ...
            'description': f'{t_name} genetic background on LAI'
        })
    for i, s in enumerate(scenarios, 1):
        log(f"    {i}. {s['name']}: {s['description']}")
    if not scenarios:
        log("  Warning: No GBK treatment groups found", level='warning')
    return scenarios
...
def run_dml_estimation(T, Y, X_conf, confounder_names, scenario_name, treatment_name,
    ...
    log(f"
{'='*60}")
    log(f"DML: {scenario_name}")
    log(f"Treatment: {treatment_name}, n={len(Y)}, p={X_conf.shape[1]}")
    log(f"{'='*60}")
    t_unique = len(np.unique(T))
    if t_unique < 2:
        log(f"  Warning: {treatment_name} has no variation ({t_unique}), attempting anyway", level='warning')
    ...
    try:
        ...
        log(f"  LinearDML failed: {e}", level='error')
    finally:
    ...
    if compute_fold_ates and len(Y) >= cv_folds * 2:
        ...
        for train_idx, test_idx in kf.split(np.arange(len(Y))):
            ...
            try:
                ...
                log(f"    Fold ATE failed: {e}", level='warning')
            finally:
...
def analyze_heterogeneous_effects(X_full, y_full, feature_names, reference_group='GBK_TEM'):
    log(f"
{'='*70}")
    log("Step 4: Heterogeneous effect analysis")
    log(f"{'='*70}")
    ...
    if not target_treatments:
        raise ValueError("No target treatments found")
    log(f"  Target treatments: {target_treatments}")
    hetero_results = []
    fold_ate_data = {}
    for treat_name in target_treatments:
        ...
        dml_res = run_dml_estimation(
            ...
            scenario_name=f"{treat_name}_hetero",
...
def plot_figure11_combined(ate_data, fold_data, reference_group='TEM', output_dir=FIGURES_DIR):
    ...
    if not ate_data_valid:
        log("  No valid ATE data, skipping.", level='warning')
        return
    ...
    markers_colors = []
    for trt in groups:
        if trt in IJoRS_COLORS:
            markers_colors.append(IJoRS_COLORS[trt])
        else:
            markers_colors.append(IJoRS_COLORS['neutral'])
    for i, (ate, color) in enumerate(zip(ates, markers_colors)):
        ax1.scatter(ate, y_pos[i], s=60, color=color, 
                   edgecolor='black', linewidth=0.5, zorder=4)
    ax1.axvline(x=0, color='black', linestyle='--', linewidth=0.7, alpha=0.8, zorder=2)
    ax1.invert_yaxis()
    x_min = min(ci_lower) - 0.15 * (max(ci_upper) - min(ci_lower))
    x_max = max(ci_upper) + 0.35 * (max(ci_upper) - min(ci_lower))
    ...
    for i, (ate, cl, cu, p) in enumerate(zip(ates, ci_lower, ci_upper, p_values)):
        ...
        if np.isnan(p):
            ...
            sig = 'ns'
        text_x = max(ci_upper) + 0.05 * (max(ci_upper) - min(ci_lower))
    ...
    ax1.set_title('(a) Average Treatment Effects with 95% CI', 
                 fontsize=IJoRS_CONFIG['font_size_title'], pad=8)
    ax1.grid(True, alpha=0.3, axis='x', linestyle=':', linewidth=0.5)
    box_data = []
    box_labels = []
    for treat in [d['group'] for d in ate_data_sorted]:
        label = treat.replace('GBK_', '')
    ...
    ax2.axvline(x=0, color='black', linestyle='--', linewidth=0.7, alpha=0.6, zorder=2)
    ax2.invert_yaxis()
    ax2.set_yticks(y_pos)
    ax2.set_yticklabels(box_labels, fontsize=IJoRS_CONFIG['font_size_tick'])
    ax2.set_xlabel('Cross-fitting ATE Estimates', fontsize=IJoRS_CONFIG['font_size_label'])
    ax2.set_title('(b) Cross-fitting Stability', 
                 fontsize=IJoRS_CONFIG['font_size_title'], pad=8)
...
def plot_heterogeneous_effects_forest(hetero_results, output_dir=FIGURES_DIR):
    ...
    if not valid_results:
        log("No valid data for forest plot", level='warning')
        return
...
(Excluding VI_NDVI_mean & DAS)', 
                 fontsize=IJoRS_CONFIG['font_size_title'], pad=10)
